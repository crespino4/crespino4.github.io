        ...
        if(data.topicName.toLowerCase() === 'channel.metadata') {
            ...
            if (data.eventBody.transcripts !== undefined && data.eventBody.transcripts.length > 0 ) {
                data.eventBody.transcripts.forEach( transcript => {
                    ...
                    var transcriptText = transcript.alternatives[0].decoratedTranscript;

                    // Wait for translate to finish before calling addChatMessage
                    translate.translateText(transcriptText, genesysCloudLanguage, function(translatedData) {
            ...
            if(publish && !messageIds.includes(messageId)) { // Make sure message is published only once
                ...
                .then((messageDetail => {
                    ...
                    translate.translateText(messageDetail.textBody, genesysCloudLanguage, function(translatedData) {
                        view.addChatMessage(name, translatedData.translatedext, purpose);
                        translationData = translatedData;
                    });
                }));
            }
