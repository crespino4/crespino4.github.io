...
async function onSocketMessage(event){
    ...
    if ( topic === topicConversation && eventBody.id === currentConversation.id ) {
        ...
        // Cache the event as the current conversation snapshot and display it.
        currentConversation = eventBody;
        document.querySelector("#conversationEvent").innerHTML = JSON.stringify(currentConversation, null, 3);

        conversationsApi.getConversation(currentConversation.id)
        .then((data) => {
...
function parseAppParameters(queryString) {
    ...
    if ( queryString.length != 0 ) {
        ...
        for (var i = 0; i < pairs.length; i++)
        {
            ...
            } else if (currParam[0] === 'defaultURL') {
                appParams.defaultURL = decodeURIComponent(currParam[1]);
            } else if (currParam[0] === 'proxyURLAttribute') {
                appParams.proxyURLAttribute = decodeURIComponent(currParam[1]);
            }
        }
    }
